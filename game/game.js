...
const player = new Player(innerWidth/2, innerHeight/2, PLAYER_SIZE, 'white');
let currentFrame;
...
// process
const play = () => {
    c.fillStyle = 'rgba(0,0,0, 0.2)'; // background
    c.fillRect(0, 0, canvas.width, canvas.height); // wipe screen from previous frame
    currentFrame = requestAnimationFrame(play); // play next frame
    player.draw();
        ...
        bullets.forEach( (bullet, bulletIndex) => {
            ...
            if (distance <= 0) { // bullet and enemy touch
                ...
                if (enemy.radius - 10 > ENEMY_SIZE_MIN) { // not dead so reduce size
                    gsap.to(enemy, {duration: 0.25, radius: enemy.radius - 10})
                } else { // enemy dead
